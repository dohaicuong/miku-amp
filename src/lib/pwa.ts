 ...
 *  - **Nuclear**: nothing is waiting yet (slow network at toast time).
 *    Unregister every SW and wipe only the `workbox-precache-*` caches, so
 *    the next navigation has no SW interception and goes straight to the
 *    network. App-data caches (`ma-images`, `ma-docs`) survive so cached
 *    art + docs stay available offline.
 */
