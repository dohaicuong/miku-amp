...
function Root({ className, ...props }: ComponentProps<typeof BaseAccordion.Root>) {
  return (
    <BaseAccordion.Root
      className={cn(
        "flex flex-col rounded-md border border-border bg-bg overflow-hidden",
        className,
      )}
      {...props}
    />
  ...
  return (
    <BaseAccordion.Trigger
      className={cn(
        ...
        "text-left text-style-track-title text-fg",
        "px-5 py-4 cursor-pointer",
        "hover:bg-surface transition-colors duration-150",
        // Negative outline-offset draws the focus ring inside the trigger.
        // With offset-2 it would render between adjacent items as a visible
        // horizontal line (Root's `overflow-hidden` clips the top/sides of
        // the outline but not the gap between items), looking like a stray
        // separator instead of a focus indicator.
...
type PanelProps = ComponentProps<typeof BaseAccordion.Panel> & {
  // The Panel itself owns the height transition; an inner div carries the
  // padding so the height collapse stays clean. `contentClassName` lets a
  // caller swap that padding (e.g. tighten it for a sidebar accordion).
  contentClassName?: string;
};

function Panel({ className, contentClassName, children, ...props }: PanelProps) {
  return (
    <BaseAccordion.Panel
      className={cn(
        "overflow-hidden",
