...
// 30 minutes is a reasonable balance for a music app — frequent enough
// that a deploy lands within a session, infrequent enough not to hammer
// the network while a user is listening.
...
export function PwaUpdateToast() {
  ...
  useEffect(() => {
    ...
    toast.add({
      ...
      description: "Reload to drop the latest cels into the player.",
      actionProps: {
        children: "Reload",
        onClick: () => {
  ...
  useEffect(() => {
    ...
    toast.add({
      ...
      description: "The player is cached. It'll boot without a connection from now on.",
      onClose: () => {
